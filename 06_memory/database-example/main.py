...
def setup_chat_chain():
    ...
    # 체인
    chain = prompt | ChatOpenAI(temperature=0.3, model="gpt-4") | StrOutputParser()
...
async def chat(request: ChatReuqest):
    try:
        config = {"configurable": {"user_id": request.user_id, "conversation_id": request.conversation_id}} # request.*** -> 딕셔너리 형태, config: 우리가 보내는 양식
